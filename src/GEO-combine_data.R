...
# all_series_metadata_name <-
#   list.files(path = paste0('data/', database_type), pattern = '_series_metadata.txt', full.names = T)
# metadata_total <-
...
# find which is 850K which is 450K
...
write.table(
  intersect_matrix,
  file = paste0('data/', database_type, '/all_betavalues_new.txt'),
  quote = F,
  row.names = F,
  sep = "\t"
)

gzip(filename=paste0('data/', database_type, '/all_betavalues_new.txt'), 
     destname=paste0('data/', database_type, '/all_betavalues_new.txt'), overwrite=TRUE, remove=TRUE, BFR.SIZE=1e+07)

#unlink(all_betavalue_name)