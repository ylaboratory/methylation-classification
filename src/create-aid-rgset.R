...
log_file <- "logs/R_aid_process_log.txt"
...
if (MAKE_RGSET){
  ...
  saveRDS(RGSet, file = "raw/GEO/RGSet_aid.rds")

} else {
  log_print("Reading RGSet...")
  
  RGSet <- readRDS("raw/GEO/RGSet_aid.rds")
...
if (MAKE_MSET){
  ...
  saveRDS(MSet_noob, file = "raw/GEO/aid_MSet.rds")
  ...
  log_print(paste(" QC Failed samples saved at: data/GEO/compiled/R_aid_qc_failed.csv"))
  write.csv(failed_samples, 'data/GEO/compiled/R_aid_qc_failed.csv')
  
  log_print(paste(" QC Passed samples saved at: data/GEO/compiled/R_aid_qc_passed.csv"))
  write.csv(MSet_noob$Sample[which((meds >= 10.5))], 'data/GEO/compiled/R_aid_qc_passed.csv')
  ...
  saveRDS(MSet_qc, file = "raw/GEO/aid_MSet_qc.rds")
} else {
  ...
  # passed_samples <- read.csv('data/GEO/compiled/R_aid_qc_passed.csv')$x
  # passed_samples <- setdiff(passed_samples, samples_to_remove)
  
  log_print("Reading MSet_qc...")
  MSet_qc <- readRDS("raw/GEO/aid_MSet_qc.rds")
  log_print(paste(" Dimensions of MSet_qc:",dim(MSet_qc)))
  
  log_print("Reading meta...")
  aid_meta <- read.csv('data/GEO/preprocessed/aid_meta.csv')
  log_print(paste(" Dimensions of aid_meta:",dim(aid_meta)))
  
  MSet_qc <- MSet_qc[, match(aid_meta$Sample, MSet_qc$Sample)]
  RGSet_qc <- RGSet[, match(aid_meta$Sample, RGSet$Sample)]
...
if (QC){
  ...
  pdf(file.path(WORK_DIR, "logs/aid_Detection_P_Values_Barplot.pdf"))
  ...
  # log_print(paste(" Dimensions of RGSet_qc:", dim(RGSet_qc)))
  log_print(paste(" Dimensions of MSet_qc:", dim(MSet_qc))) 
  write.csv(rownames(MSet_qc), "annotation/aid_detP_probes.csv", row.names = FALSE)
} else {
  detP_probes <- read.csv("annotation/aid_detP_probes.csv")$x
...
saveRDS(RSet, file = "raw/GEO/aid_RSet.rds")
...
saveRDS(GRSet_bmiq, file = "raw/GEO/aid_GRSet_bmiq.rds")
...
# log_print(paste(" Dimensions of GRSet_bmiq: ", paste(dim(GRSet_bmiq), collapse = " x ")))
# saveRDS(GRSet_bmiq, file = "raw/GEO/aid_GRSet_bmiq.rds")
# GRSet_bmiq <- readRDS("raw/GEO/aid_GRSet_bmiq.rds")
...
saveRDS(GRSet_flt, file = "raw/GEO/aid_GRSet_flt.rds")
