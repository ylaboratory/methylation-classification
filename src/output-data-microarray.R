...
write_to_file<-function(GRset_BMIQ_genome_loci_lift, dir2metadata, out_directory='data/') {
  ...
  #     sep = '\t'
  #   )
  write.table(GRset_BMIQ_genome_loci_lift, paste0(out_directory, metadata_table[1,'Database'],'/', metadata_table[1,'Series'], '_beta_values.txt'),
              quote = F,
              sep = '\t', row.names = F)
  }
