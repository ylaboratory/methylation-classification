...
logging.basicConfig(
    ...
    filemode="w"  # Overwrites existing file
)
...
def get_significant_k_probes(res_per_tissue, k=30):
...
def get_top_k_probes(res_per_tissue, k=30):
    """Get the top k most significant probes for each tissue based on PValue."""
    return {
        tissue: list(tissue_res.sort_values("PValue").head(k).index)
        for tissue, tissue_res in res_per_tissue.items()
    }
...
def batch_correlation(holdout_Mv, rest_Mv, rest_meta, probe_per_tissue):
    ...
    for tissue, probes in tqdm(probe_per_tissue.items(), desc="Processing Tissues", unit="tissue"):
        ...
        n_holdout = holdout_data.shape[0] #axis change
        logger.info(f"{tissue}: {n_holdout}, {corr_matrix.shape}...")

        corr_matrix = corr_matrix[:n_holdout, n_holdout:]  # (N_holdout_samples, N_tissue_samples) #axis change
        logger.info(f"{tissue}: {n_holdout}, {corr_matrix.shape}...")
...
def main():
    ...
    for fold, (rest_Mv, rest_meta, holdout_Mv, holdout_meta) in fold_Mvs.items():
        ...
        probe_per_tissue = get_significant_k_probes(res_per_tissue, k=k)
