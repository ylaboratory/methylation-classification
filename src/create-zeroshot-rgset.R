...
DATABASE_TYPE <- 'GEO'
# META_FILE <- 'data/GEO/preprocessed/zeroshot_meta.csv'
# META_FILE <- "annotation/filtered_zeroshot_meta.csv"
# META_FILE <- "annotation/combined_metadata.csv"
RAW_DATA_DIR <- 'raw/GEO_zeroshot'
MAKE_RGSET <- TRUE
MAKE_MSET <- TRUE
QC <- FALSE
...
log_file <- "logs/R_zeroshot_process_log.txt"
...
# Create zeroshot metadata
log_print("Reading metadata...")
meta_all <- read.csv('annotation/meta_gse_unique_idats.csv')
dim(meta_all)
meta_all <- meta_all[meta_all$idat_check=='True', ]
dim(meta_all)
meta_all <- meta_all[meta_all$normal=='normal', ]
dim(meta_all)
meta_all <- meta_all[meta_all$treatment=='False', ]
dim(meta_all)
training_meta <- read.csv('annotation/training_meta.csv')
zeroshot_potential <- meta_all[!meta_all$sample %in% training_meta$Sample,]
dim(zeroshot_potential)
zeroshot_potential$X <- NULL
colnames(zeroshot_potential) <- c("Dataset","Disease","Normal","Platform","Sample","Annotated.tissue","Treatment","idat_check")
write.csv(zeroshot_potential, 'annotation/zeroshot_potential.csv', row.names = FALSE)

training_preqc <- read.csv('_annotation/combined_metadata.csv')
dim(training_preqc)

additional_total_found_existing <- read.csv('_annotation/additional_total_found.csv')$x
additional_total_found_all <- unlist(lapply(list.files("data/GEO/compiled/qc_failed_samples/", pattern = "\\.csv$"), 
                                            function(f) read.csv(file.path("data/GEO/compiled/qc_failed_samples/", f))$sample))
...
if (MAKE_RGSET){
  ...
  file_match <- filter_files(all_files, zeroshot_potential)

  grn_files <- file_match$grn_files
  red_files <- file_match$red_files
  
  log_print(paste("of", dim(zeroshot_potential)[1], "zeroshot files", length(file_match$grn_files)[1], "found"))
  ...
  # Create RGSet
  # may need to 
  # cd ./../raw/GEO_zeroshot/GSE107205/
  # for file in *_Gn.idat.gz; do mv "$file" "${file/_Gn.idat.gz/_Grn.idat.gz}"; done
  log_print("Creating RGSet...")
  # RGSet <- read.metharray.exp(targets = sample_files)
  
  # get_failed_samples <- function(files) {
  #   failed_indices<- integer()
  #   pb <- txtProgressBar(min = 0, max = length(files[,1]), style = 3)  # Progress bar
  #   for (i in seq_along(files[,1])) {
  #     tryCatch({
  #       RGSet <- read.metharray.exp(targets = files[i,])
  #     }, error = function(e) {
  #       failed_indices <<- c(failed_indices, i)  # Store failed index
  #     })
  ...
  # }
  # failed_indices <- get_failed_samples(sample_files)
  failed_indices <- c(398, 3036)
  load_failed_samples <- sample_files[failed_indices,]
  if (length(load_failed_samples) > 0) print(load_failed_samples)
  sample_files <- sample_files[-failed_indices,]
  
  RGSet <- read.metharray.exp(targets = sample_files)

  log_print(" Saving RGSet...")
  saveRDS(RGSet, file = "raw/GEO/zeroshot_RGSet.rds")
  log_print(paste("n sample files:", nrow(sample_files)))
} else {
  log_print("Reading RGSet...")
  # RGSet_add <- readRDS("raw/GEO/zeroshot_RGSet_additional.rds")
  # RGSet <- readRDS("raw/GEO/zeroshot_RGSet.rds")
  ...
  # saveRDS(RGSet, file = "raw/GEO/zeroshot_RGSet_added.rds")
  
  RGSet <- readRDS("raw/GEO/zeroshot_RGSet_added.rds")
  log_print(paste(" Dimensions of RGSet: ",dim(RGSet)))
}
...
if (MAKE_MSET){
  ...
  saveRDS(MSet_noob, file = "raw/GEO/zeroshot_MSet.rds")
  ...
  log_print(paste(" QC Failed samples saved at: data/GEO/compiled/R_zeroshot_qc_failed.csv"))
  write.csv(failed_samples, 'data/GEO/compiled/R_zeroshot_qc_failed.csv')
  
  log_print(paste(" QC Passed samples saved at: data/GEO/compiled/R_zeroshot_qc_passed.csv"))
  write.csv(MSet_noob$Sample[which((meds >= 10.5))], 'data/GEO/compiled/R_zeroshot_qc_passed.csv')
  ...
  saveRDS(MSet_qc, file = "raw/GEO/zeroshot_MSet_qc.rds")
} else {
  ...
  # passed_samples <- read.csv('data/GEO/compiled/R_zeroshot_qc_passed.csv')$x
  # passed_samples <- setdiff(passed_samples, samples_to_remove)
  
  log_print("Reading MSet_qc...")
  MSet_qc <- readRDS("raw/GEO/zeroshot_MSet_qc.rds")
  log_print(paste(" Dimensions of MSet_qc:",dim(MSet_qc)))
  
  log_print("Reading traininig_meta...")
  zeroshot_meta <- read.csv('data/GEO/preprocessed/zeroshot_meta.csv')
  log_print(paste(" Dimensions of zeroshot_meta:",dim(zeroshot_meta)))
  
  MSet_qc <- MSet_qc[, match(zeroshot_meta$Sample, MSet_qc$Sample)]
  RGSet_qc <- RGSet[, match(zeroshot_meta$Sample, RGSet$Sample)]
...
saveRDS(RSet, file = "raw/GEO/zeroshot_RSet.rds")
...
saveRDS(GRSet_bmiq, file = "raw/GEO/zeroshot_GRSet_bmiq.rds")
...
# log_print(paste(" Dimensions of GRSet_bmiq: ", paste(dim(GRSet_bmiq), collapse = " x ")))
# saveRDS(GRSet_bmiq, file = "raw/GEO/zeroshot_GRSet_bmiq.rds")
# GRSet_bmiq <- readRDS("raw/GEO/zeroshot_GRSet_bmiq.rds")
...
saveRDS(GRSet_flt, file = "raw/GEO/zeroshot_GRSet_flt.rds")
