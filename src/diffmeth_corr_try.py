...
# Configure logging
logging.basicConfig(filename="diffmeth_corr_try.log",level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
...
def batch_correlation(holdout_Mv, rest_Mv, rest_meta, probe_per_tissue, save=False):
    ...
    for tissue, probes in tqdm(probe_per_tissue.items(), desc="Processing Tissues", unit="tissue"):
        ...
        if (save) and (tissue in [name_to_id[x][0] for x in ['airway','esophagus','bone tissue','visceral fat']]):
            pd.DataFrame(corr_matrix, index=list(holdout_data.index)+list(tissue_data.index), columns=list(holdout_data.index)+list(tissue_data.index)).to_csv(f'./../extra/holdout_x_{id_to_name[tissue][0]}.csv')
...
def main():
    ...
    for fold, (rest_Mv, rest_meta, holdout_Mv, holdout_meta) in fold_Mvs.items():
        ...
        if fold==0:
            sample_corrs = batch_correlation(holdout_Mv, rest_Mv, rest_meta, probe_per_tissue, save=True)
        else:
            sample_corrs = batch_correlation(holdout_Mv, rest_Mv, rest_meta, probe_per_tissue, save=False)
        ...
        fold_res_df.to_pickle(f'{DATA_PATH}/diffmeth/diffmeth_corr_try_f{fold}.pkl')

        break

    # Save final results
    final_res_df = pd.DataFrame.from_dict(res_dict, orient='index', columns=['corr', 'pred', 'true', 'dict'])
    final_res_df.to_pickle(f'{DATA_PATH}/diffmeth/diffmeth_corr_try.pkl')
