...
annot<-function(total_metadata, meta_dir, sample, disease_vec, tissue_vec, 
  ...
  if (dim(sample_matrix)[1]!=length(disease_vec) || dim(sample_matrix)[1]!=length(tissue_vec) || dim(sample_matrix)[1]!=length(treatment_vec)){
    print(sample)
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE108202', 
                      ...
                      c(rep('False',8)))
total_metadata<-annot(total_metadata, meta_dir, 'GSE108785', c(rep('disease',3), rep('normal',3)), c(rep('Blood', 6)), c(rep('False',6)))
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE100386', c(rep('normal',46)), c(rep('Peripheral Blood Mononuclear Cell',46)),
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE100561', c(rep(c(rep('normal',3), rep('disease',3)),2)), c(rep('Monocyte',12)),c(rep('False',12)))
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE101443', c(rep(c('disease','normal'),4)), c(rep(c('Breast Neoplasm','Breast'),4)), 
                      c(rep('False',8)))
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE102177', c(rep('normal',18), rep('disease',18)), c(rep('Blood',36)),c(rep('False',36)))
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE103006', c(rep('normal',24)),c(rep('Umbilical Blood',24)), c(rep('True',24)))
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE103768', c(rep('normal',57)), c(rep('Adipose Tissue',57)), c(rep('False',57))) #obesity study over time
total_metadata<-annot(total_metadata, meta_dir, 'GSE103911', c(rep('disease', 27),rep('normal',27), rep('disease', 11)), 
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE104376', c(rep('normal',45)), c(rep('Umbilical Blood',45)), 
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE104812', c(rep('normal',48)), c(rep('Blood',48)), c(rep('False',48)))
to_remove<-append_to_remove(to_remove, 'GSE105066')
total_metadata<-annot(total_metadata, meta_dir, 'GSE105123', c(rep('normal',108)), c(rep('Peripheral Blood Mononuclear Cell',108)), 
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE105260', c(rep('disease',26),rep('normal',9),rep('disease',9)),
                      ...
                      c(rep('False',44)))
total_metadata<-annot(total_metadata, meta_dir, 'GSE106089', c(rep('normal',84)),c(rep('Placenta',84)), c(rep('False',84))) #low gestational age newborn study
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE107211', c(rep('normal',15)),c(rep('Blood',15)),c(rep('False',15)))
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE107351', 
                      ...
                      c(rep('Blood',41),rep('Lynch Syndrome',61),rep('MLH1 Gene Mutation',14)),
                      c(rep('False',116)))
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE107737', 
                      ...
                      c(rep('Blood',12),rep('Hypopituitarism',12)),
                      c(rep('False',24)))
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE109430', 
                      ...
                      c(rep('Leukocyte',12),rep('Kawasaki Disease',24)), 
                      c(rep('False',36)))
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE109905', 
                      ...
                      c(rep('Atrial Septal Defect', 38), rep('Blood',31)),
                      c(rep('False',69)))
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE112306', c(rep('normal',6)),c(rep('Epithelial Cell',3),rep('Mesenchymal',3)),c(rep('True',6)))
total_metadata<-annot(total_metadata, meta_dir, 'GSE112314', c(rep('normal',22)),c(rep('Saliva',22)),c(rep('False',22))) #range of childhood stress
total_metadata<-annot(total_metadata, meta_dir, 'GSE112696', 
                      c(rep('normal',6),rep('disease',6)), 
                      c(rep('T-Lymphocyte',6),rep('Myasthenia Gravis',6)),
                      c(rep('False',12)))
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE115797', 
                      c(rep(list('normal','disease'),24)), 
                      c(rep(list('Skin','Psoriasis'),24)),
                      c(rep('False',48)))
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE119778', 
                      ...
                      c(rep('Williams Syndrome',34),rep('Blood',34)),
                      c(rep('False',68)))
total_metadata<-annot(total_metadata, meta_dir, 'GSE119846', c(rep('normal', 60)), c(rep('Hematopoietic Stem Cell',60)), c(rep('True',60)))
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE126017', 
                      ...
                      c(rep('Spermatozoon',18), rep('Psoriasis',36)), 
                      c(rep('False',54)))
total_metadata<-annot(total_metadata, meta_dir, 'GSE128126', c(rep('normal',15)), c(rep('Embryonic Stem Cell',15)))
total_metadata<-annot(total_metadata, meta_dir, 'GSE127824', c(rep('normal',24)), c(rep('Umbilical Blood',24)), c(rep('False',24)))
total_metadata<-annot(total_metadata, meta_dir, 'GSE128801', c(rep('disease',12)), c(rep('Blood',12)), c(rep('False',12)))
total_metadata<-annot(total_metadata, meta_dir, 'GSE129266', c(rep('normal',12)), c(rep('Mesenchymal Stem Cell',12)), c(rep('False',12))) #timeseries
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE132399', 
                      ...
                      c(rep('Liver', 26), rep('Hepatoblastoma',28)),
                      c(rep('True',5), rep('False',49)))
total_metadata<-annot(total_metadata, meta_dir, 'GSE137716', 
                      c(rep('normal',16),rep('disease',17)), 
                      c(rep('Tracheal Epithelium',16),rep('Asthma',17)),
                      c(rep('False',33)))
total_metadata<-annot(total_metadata, meta_dir, 'GSE138279', c(rep('normal',65)), c(rep('Saliva',65)), c(rep('False',65))) #childhood trauma
total_metadata<-annot(total_metadata, meta_dir, 'GSE139307', c(rep('normal',37)), c(rep('Spermatozoon',37)) , c(rep('False',37))) #dioxin exposure
total_metadata<-annot(total_metadata, meta_dir, 'GSE141338', 
                      c(rep('normal',6),rep('disease',42)), 
                      c(rep('Breast',6),rep('Breast Neoplasm',42)),
                      c(rep('False',48)))
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE147740', c(rep('normal',1129)), c(rep('Blood',1129))) #wide variety
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE151042', c(rep('normal',492)), c(rep('Umbilical Blood',492)))
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE48472', 
                      ...
                      c(rep('Blood',6),rep('Liver',5),rep('Muscle',6),rep('Omentum',6),rep('Pancreas',4),
                        rep('Adipose Tissue',6),rep('Spleen',3),rep('Blood',5),rep('Buccal Mucosa',5),
                        rep('Hair',5),rep('Saliva',5)))
total_metadata<-annot(total_metadata, meta_dir, 'GSE49618', c(rep('normal',21)), c(rep('Bone Marrow',21))) #different stem/immune celltypes
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE59524', c(rep('normal',24)), c(rep('Adipose Tissue',24))) #visceral
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE61278', c(rep('normal', 66), rep('disease',44)), c(rep('Liver', 110))) #mix of fetal and adult
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE64096', c(rep('normal',40)), c(rep('Spermatozoon',40))) #two different density
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE75133', c(rep('normal',15)), c(rep('Mesenchymal Stem Cell',15)), c(rep('True',15)))
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE75704', 
                      ...
                      c(rep('Frontal Lobe Cortex',72), rep('Progressive Supranuclear Palsy', 94))) #post mortem
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE79329', c(rep('normal',34)), c(rep('Leukocyte',34))) #pollutant study
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE82234', c(rep('normal',6)), c(rep('Umbilical Blood',6))) #different passages
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE83944', c(rep('normal',48)), c(rep('Neutrophil',48))) #same sample, different time of day
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE85042', c(rep('normal',71)), c(rep('Umbilical Blood',71))) #gestational stimuli study
total_metadata<-annot(total_metadata, meta_dir, 'GSE85210', 
                      c(rep('normal',253)), 
                      c(rep('Blood',253))) #smoking study
total_metadata<-annot(total_metadata, meta_dir, 'GSE85506', c(rep('disease',47)), c(rep('Blood',47)))
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE87582', c(rep('disease',21)), c(rep('Blood',21)))
total_metadata<-annot(total_metadata, meta_dir, 'GSE87640', c(rep('disease', 240)), c(rep('Leukocyte',240))) #complicated, some healthy mixed
total_metadata<-annot(total_metadata, meta_dir, 'GSE88883', c(rep('normal',100)), c(rep('Breast',100)))
total_metadata<-annot(total_metadata, meta_dir, 'GSE89218', c(rep('normal',163)), c(rep('Blood',163))) #PTSD study
total_metadata<-annot(total_metadata, meta_dir, 'GSE89251', c(rep('disease',136)), c(rep('T-Lymphocyte',136)))
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE89852', c(rep('normal',37), rep('disease',37)), c(rep('Liver', 74)))
total_metadata<-annot(total_metadata, meta_dir, 'GSE89925', c(rep('normal',3),rep('disease',3)), c(rep('Fibroblast',3),rep('Teratoma',3)))
total_metadata<-annot(total_metadata, meta_dir, 'GSE90060', c(rep('normal', 34)), c(rep('Endometrium',34))) #differen part of implanting
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE99755', c(rep('normal',67)), c(rep('Blood',67))) #police PTSD
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE110724', 
                      c(rep('normal',21)),
                      c(rep('Subcutis', 6), rep('Omentum', 6), rep('Adipose Tissue', 3), rep('Leukocyte',6)))
...
total_metadata<-annot(total_metadata, meta_dir, 'GSE114753', 
                      c(rep('normal',156)), 
                      c(rep('Spermatogenic Cell', 156))) #smoking study
...
write.csv(df, file=paste0('data/', database_type, '/all_metadata_annotated_may2022.txt'),
          row.names=F,
          sep="\t",
          quote=F)
          
gzip(filename=paste0('data/', database_type, '/all_metadata_annotated_may2022.txt'), 
     destname=paste0('data/', database_type, '/all_metadata_annotated_may2022.txt.gz'), overwrite=TRUE, remove=TRUE)
