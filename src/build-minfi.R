...
manifest_file <- "GEO_microarray_accession_450K.txt"
database_type <- "GEO"
...
accession_list <- read.table("annotation/450K_atleast2_series.csv", header = F)
sample_list <- read.table("annotation/450K_atleast2_sample.csv", header = F)
probe_list <- read.table("annotation/450K_atleast2_probe_highvar.csv", header = F)
...
write.table(datatable_filtered, paste0('data/', database_type,'/compiled/450K_betavalues_probe_R.csv'),
            quote = F,
            sep = '\t', row.names = T, col.names = T)

gzip(filename=paste0('data/', database_type,'/compiled/450K_betavalues_probe_R.csv'), 
     destname=paste0('data/', database_type,'/compiled/450K_betavalues_probe_R.csv.gz'), 
