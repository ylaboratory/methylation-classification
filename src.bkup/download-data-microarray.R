...
get_metadata_TCGA<-function(file2bar, output_directory='data/TCGA'){
  wanted_attributes<-c('barcode','patient','definition','sample_id','sample_type_id','primary_site','name')
  for (i in unique(file2bar[,'associated_entities.entity_submitter_id'])){
    sample<-colDataPrepare(i)
    extracted_info<-sample[wanted_attributes]
    extracted_info$primary_site<-paste(extracted_info$primary_site, collapse = " ") #turn list of primary sites into one string
    file_name<-as.character(file2bar[which(file2bar[,'associated_entities.entity_submitter_id']==i), 'file_id'])
    colnames(extracted_info)<-wanted_attributes
